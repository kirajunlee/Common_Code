...
// 逻辑处理
// ============================================================
assign llist_ctrl_ram_ren   = llist_push_en ;
assign llist_ctrl_ram_raddr =  

//push和pop操作;
assign push_en = llist_push_en & ~llist_full  ; //真实的push动作,需要链表非满;链表初始化时禁止push操作;
assign pop_en  = llist_pop_en  & ~llist_empty ; //真实的pop动作,需要链表非空;此外链表初始化允许链表排空;
//链表状态维护
//Linklist depth状态的维护;
add_sub_cnt #(
...
assign llist_pop_cur_ptr  = (pop_en & d1_pop_en)? llist_pop_nxt_ptr : llist_head ; //pop_en时头指针为pop的当前指针;如果上个cycle就已发起了pop的操作,当前cycle还发起的pop的操作,则需要使用llist_pop_nxt_ptr作为新的head指针;
